...
import { parseClovaAnseong } from '../src/lib/clova-parse-lgu-anseong';
import type { ClovaResponse } from '../src/lib/clova-ocr';

function main() {
  const arg = process.argv[2];
  if (!arg) { console.error('usage: tsx test-clova-parse-anseong.ts <json>'); process.exit(1); }
  const resp = JSON.parse(readFileSync(path.resolve(arg), 'utf8')) as ClovaResponse;
  const result = parseClovaAnseong(resp);
