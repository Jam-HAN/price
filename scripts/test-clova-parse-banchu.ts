...
import { parseClovaBanchu } from '../src/lib/clova-parse-kt-banchu';
import type { ClovaResponse } from '../src/lib/clova-ocr';

function main() {
  const arg = process.argv[2];
  if (!arg) { console.error('usage: tsx test-clova-parse-banchu.ts <json>'); process.exit(1); }
  const resp = JSON.parse(readFileSync(path.resolve(arg), 'utf8')) as ClovaResponse;
  const result = parseClovaBanchu(resp);
  ...
  for (const m of result.models.slice(0, 12)) {
    ...
    for (const t of m.tiers) {
      const c = t.common!;
      console.log(
        `   ${t.plan_tier_code.padEnd(6)} 공시 ${(t.subsidy_krw ?? 0).toLocaleString().padStart(8)} | ` +
